# Hosted Fields example — Node.js + Express + plain JS
...
Open <http://localhost:3000/hosted-fields-examples-nodejs-express-js/>.
...
from it and are worth knowing when reading [src/paynet.js](src/paynet.js):
...
esbuild bundles the server and its only dependency into a single file. The artefact needs no
`node_modules` and no `package.json` on the target host — just Node 20+:
...
npm run build
install -d -o hosted-fields -g hosted-fields /opt/hosted-fields-examples-nodejs-express-js
rsync -a --delete dist/ /opt/hosted-fields-examples-nodejs-express-js/

# 2. the RSA key as a file, not an env variable
install -m 640 -o root -g hosted-fields private_key.pem /etc/hosted-fields-examples-nodejs-express-js.key

# 3. settings
cp deploy/hosted-fields-examples-nodejs-express-js.env.example /etc/hosted-fields-examples-nodejs-express-js.env
chmod 640 /etc/hosted-fields-examples-nodejs-express-js.env   # then fill in ENDPOINT_ID, MERCHANT_LOGIN, PUBLIC_URL

# 4. service
cp deploy/hosted-fields-examples-nodejs-express-js.service /etc/systemd/system/
systemctl daemon-reload && systemctl enable --now hosted-fields-examples-nodejs-express-js

# 5. nginx
cp deploy/nginx.conf /etc/nginx/snippets/hosted-fields-examples-nodejs-express-js.conf
...
src/server.js    express routes under BASE_PATH
