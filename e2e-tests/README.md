...
needs a browser, two bundler builds and either ten toolchains or Docker, which is more
...
- **`npm test` rebuilds `nextjs/.next`**, because `basePath` is baked in at build time, **and
  `go-react/web/dist`**, because `//go:embed` needs it before the compiler runs. Do not run the
  suite while `next dev` or `rsbuild dev` is live on the same directory. Neither is installed for
  you: the two React examples expect a `yarn install` of their own. The docker mode builds both
  inside the images and leaves the working tree alone.
- **The Go examples are compiled to `.tmp/` and exec'd**, not run with `go run .`: `go run` leaves
  the compiled binary behind as a grandchild that keeps holding the port. Set `GO_BIN` if your Go
  is not at `~/opt/go/bin/go` or on `PATH`. `go-react` builds its page first, in its own
  `web/dist`, because that is what `//go:embed` compiles in.
