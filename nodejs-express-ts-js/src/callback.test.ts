...
import { stubSettings } from './settings-stub.ts';

stubSettings();
const { SIGNED_CALLBACK_FIELDS, validCallback } = await import('./callback.ts');
...
// The one test with no counterpart next door. req.query hands a repeated parameter over as an
// array, and validCallback takes req.query directly, so the case is reachable — it has to fail
// the checksum rather than throw on its way through.
test('a repeated query parameter is not silently joined', () => {
  assert.equal(validCallback({ ...CALLBACK, orderid: ['1234567', 'other'] }), false);
});
