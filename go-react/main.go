...
// The whole browser half is one directory of build output: web/ is a React application and
// `yarn build` writes the two pages, the bundle and a copy of the stylesheet into web/dist.
// `all:` keeps the files a bundler is entitled to start with a dot. web/dist/.gitkeep is
// committed so that a fresh clone compiles before anyone has run the build.
//
//go:embed all:web/dist
var assets embed.FS

// The built browser half: web/dist, as its own root.
var bundle fs.FS
...
func main() {
	...
	bundle, err = fs.Sub(assets, "web/dist")
	if err != nil {
		log.Fatal(err)
	}
	...
	// The two pages are static files that Rsbuild wrote, and this server substitutes nothing
	// into them: the only thing it generates is window.CONFIG, which it hands over as a script
	// of its own. That is what keeps BASE_PATH a setting of this process rather than something
	// baked into the bundle, the way next build bakes it into nextjs/.
	...
	// The stylesheet and the bundle. Registering the subtree also makes the mux redirect a
	// request for the bare prefix to the trailing-slash form, which keeps the relative asset
	// URLs on both pages working.
	mux.HandleFunc("GET "+cfg.BasePath+"/", handleAsset)
...
func handleCheckout(w http.ResponseWriter, r *http.Request) { serveView(w, "index.html") }
...
// handleAsset serves the parts of the bundle a browser may ask for, and nothing else. go-js can
// hand the whole of public/ to a file server; here the two pages sit in the same directory as
// the assets, so doing that would also serve {prefix}/result.html — straight past the signature
// check GET {prefix}/result performs. Hence an allowlist, the same one ruby-sinatra-js,
// java-springboot-js, kotlin-ktor-js and dotnet-aspnetcore-js keep for their own reasons.
func handleAsset(w http.ResponseWriter, r *http.Request) {
	// path.Clean first, and the cleaned name is the one served. The mux turns a literal ".."
	// into a redirect but leaves an escaped one alone, and a file server resolves it anyway —
	// so a name checked as it arrived would let {prefix}/static/%2e%2e/result.html through.
	name := strings.TrimPrefix(path.Clean(r.URL.Path), cfg.BasePath+"/")
	if name != "styles.css" && !strings.HasPrefix(name, "static/") {
		http.NotFound(w, r)
		return
	}
	http.ServeFileFS(w, r, bundle, name)
}

// Both pages are served straight out of the embedded build output, with nothing substituted
// into them.
func serveView(w http.ResponseWriter, name string) {
	page, err := fs.ReadFile(bundle, name)
	if err != nil {
		log.Printf("[error] %v", err)
...
//
// No 'unsafe-inline' anywhere, which is what the two HTML templates in web/src/app/ are written
// against: they carry no script of their own but config.js, and Rsbuild is told to inline
// neither scripts nor styles. Nothing here is templated, so there is nowhere to put a nonce —
// which is the one thing nextjs/ has to do differently, because Next emits inline scripts.
