...
import { getOrderStatus, type OrderStatusData } from '@/shared/api';
 ...
 *
 * The order does not outlive the page: after the 3DS redirect the return page reads it out of
 * the query the callback redirected to, which the server has already checked the gateway's
 * signature over. Nothing is kept in sessionStorage and nothing is taken on the browser's word.
 */
export function useOrderStatus(order: Order | null): OrderStatusData | null {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      attempts += 1;
      try {
        const data = await getOrderStatus(basePath, orderId, clientOrderId);
        if (!active) {
          return;
        }
