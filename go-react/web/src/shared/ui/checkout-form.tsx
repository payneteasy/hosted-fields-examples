...
import { type OrderStatusData, postSale } from '@/shared/api';
import type { CheckoutConfig } from '@/shared/config';
import {
  browserInfo,
...
/* The config is read from window.CONFIG by the entry module and passed in as a prop, so
   everything below is ordinary React with no global in sight — see src/shared/config. */
 ...
 *  the original, and the payer must read the same words in every example. */
...
export function CheckoutForm({ config }: { config: CheckoutConfig }) {
  ...
  useEffect(() => {
    ...
    async function sendTokenToServer(hostedFieldsToken: string) {
      try {
        const sale = await postSale(config.basePath, {
          hostedFieldsToken,
          browser: browserInfo(),
